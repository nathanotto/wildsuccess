...
const VALUE_LAYOUT = (() => {
  ...
  // Protect: arc from ~150° to ~210° (left side, top to bottom)
  protect.forEach((v, i) => {
    const angle = (Math.PI * 0.72) + (i / (protect.length - 1)) * (Math.PI * 0.56);
    positions[v.id] = {
  ...
  // Expand: arc from ~330° to ~30° (right side, top to bottom)
  expand.forEach((v, i) => {
    const angle = (Math.PI * -0.28) + (i / (expand.length - 1)) * (Math.PI * 0.56);
    positions[v.id] = {
      x: CX + Math.cos(angle) * radius,
      y: CY + Math.sin(angle) * radius * 0.85,
    };
...
export default function WildSuccessMap5() {
  ...
  return (
    <div style={{
      ...
      >
        <svg viewBox="0 0 1000 680" style={{ width: "100%", maxWidth: 1000, height: "auto" }}>
          <defs>
          ...
          <text x={CX - 200} y={50} textAnchor="middle"
            fontSize={10} fontWeight={700} fill="#9E6A46" letterSpacing="2" opacity={0.5}>
            PROTECT
          </text>
          <text x={CX + 200} y={50} textAnchor="middle"
            fontSize={10} fontWeight={700} fill="#4B82AF" letterSpacing="2" opacity={0.5}>
            EXPAND
          </text>
          ...
          <path
            d={`M ${CX} ${CY - 42} A 42 42 0 0 0 ${CX} ${CY + 42}`}
            fill="none" stroke="#9E6A46" strokeWidth={5}
            strokeOpacity={protectAvg / 10} strokeLinecap="round"
          />
          {/* Expand arc (right half) */}
          <path
            d={`M ${CX} ${CY - 42} A 42 42 0 0 1 ${CX} ${CY + 42}`}
            fill="none" stroke="#4B82AF" strokeWidth={5}
            strokeOpacity={expandAvg / 10} strokeLinecap="round"
          />
          <text x={CX} y={CY - 8} textAnchor="middle" fontSize={14} fontWeight={700} fill="#2D2A26">
            Nathan
          </text>
          <text x={CX - 18} y={CY + 10} textAnchor="middle" fontSize={9} fontWeight={600} fill="#9E6A46">
            {protectAvg.toFixed(1)}
          </text>
          <text x={CX + 18} y={CY + 10} textAnchor="middle" fontSize={9} fontWeight={600} fill="#4B82AF">
            {expandAvg.toFixed(1)}
          </text>
          {/* Highest leverage text */}
          <text x={CX} y={CY + 26} textAnchor="middle" fontSize={7.5} fontWeight={500}
            fill="#C4504A" opacity={0.8}>
            {getHighestLeverage()}
          </text>
          ...
          {VALUES.map(v => {
            ...
            return (
              ...
              >
                {(isSel || isHl) && (
                  <circle cx={vp.x} cy={vp.y} r={r + 6}
                    fill="none" stroke={vc.stroke} strokeWidth={1.5}
                ...
                {Array.from({ length: Math.min(actCount, 6) }, (_, i) => {
                  ...
                  return (
                    <circle key={i}
                      cx={vp.x + Math.cos(dotAngle) * (r + 6)}
                      cy={vp.y + Math.sin(dotAngle) * (r + 6)}
                      r={2} fill={vc.fill} opacity={0.4}
                    />
                  );
                })}
                {/* Name */}
                <text x={vp.x} y={vp.y + r + 13} textAnchor="middle"
                  fontSize={10} fontWeight={700}
                  fill={below ? "#C4504A" : "#2D2A26"}>
                  {v.name}
                </text>
                {/* Status */}
                <text x={vp.x} y={vp.y + r + 24} textAnchor="middle"
          ...
          {ACTIVITIES.map(a => {
            ...
            return (
              ...
              >
                ...
                {(isHl || isSel || isHov) && (
                  <text x={ap.x} y={ap.y - 10} textAnchor="middle"
                    fontSize={8} fontWeight={a.overdue ? 700 : 500}
          ...
          {ACTIVITIES.filter(a => a.overdue).map(a => {
            ...
            return (
              <text key={`lbl-${a.id}`} x={ap.x} y={ap.y - 10} textAnchor="middle"
                fontSize={7.5} fontWeight={600} fill="#C4504A" opacity={0.7}>
                {a.name}
              </text>
            );
          })}
